...
def compute_AAMI_performance_measures(predictions, gt_labels):
    ...
    for i in range(0, n_classes):
        ...
        pf_ms.Recall[i] = TP / (TP + FN) # Sen
        pf_ms.Precision[i] = TP / (TP + FP) # Ppr
        pf_ms.Specificity[i] = TN / (TN + FP);  # Spe
        pf_ms.Acc[i] = (TP + TN) / (TP + TN + FP + FN) # ACC
...
def write_AAMI_results(performance_measures, filename):
    ...
    f.write("Ppr: " + str(format(performance_measures.Precision[0], '.4f')) + "\n")
    f.write("Spe: " + str(format(performance_measures.Specificity[0], '.4f')) + "\n")
    f.write("Acc: " + str(format(performance_measures.Acc[0], '.4f')) + "\n")
    
    f.write("SVEB:" + "\n\n")
    f.write("Sens: " + str(format(performance_measures.Recall[1], '.4f')) + "\n")
    f.write("Ppr: " + str(format(performance_measures.Precision[1], '.4f')) + "\n")
    f.write("Spe: " + str(format(performance_measures.Specificity[1], '.4f')) + "\n")
    f.write("Acc: " + str(format(performance_measures.Acc[1], '.4f')) + "\n")

    f.write("VEB:" + "\n\n")
    f.write("Sens: " + str(format(performance_measures.Recall[2], '.4f')) + "\n")
    f.write("Ppr: " + str(format(performance_measures.Precision[2], '.4f')) + "\n")
    f.write("Spe: " + str(format(performance_measures.Specificity[2], '.4f')) + "\n")
